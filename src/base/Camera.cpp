...
void Camera::SetCameraRotaX_outside(FLOAT xValue){
	cameraX += xValue;
	if(cameraX > PHI)cameraX -= twoPHI;
	else if(cameraX < -PHI)cameraX += twoPHI;
}
...
void Camera::SetCameraRotaY_inside(FLOAT yValue){
	cameraY += yValue;
	if(cameraY >= hPHI-0.1f)cameraY = hPHI-0.1f;
	else if(cameraY <= -hPHI+0.1f)cameraY = -hPHI+0.1f;
}
...
void Camera::SetCameraIdx(ONEOF_PI x,ONEOF_PI y,ONEOF_PI z)
{
	posidx.x = x;
	posidx.y = y;
	posidx.z = z;
...
void Camera::DebugDraw(DebugCommentManager* pDebugM)
{
	...
	_stprintf_s(txt,699,_T("Camera\n")
						...
						_T("X:%f/Y:%f\nlen:%f\n")
						_T("posidx..X:%lld/Y:%lld/Z:%lld"),
		pos.x,pos.y,pos.z,
		v_pos.x,v_pos.y,v_pos.z,
		dir.x,dir.y,dir.z,
		cameraX,cameraY,len,
		posidx.x,posidx.y,posidx.z
		);
	pDebugM->Set(txt,8);
	pDebugM->End();
}
